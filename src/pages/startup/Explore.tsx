    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-3">
          <input className="border rounded-md px-3 py-2 text-sm" placeholder="Search name or description" value={search} onChange={e => setSearch(e.target.value)} />
          ...
          <input className="border rounded-md px-3 py-2 text-sm" placeholder="Tags (comma-separated)" value={tagQuery} onChange={e => setTagQuery(e.target.value)} />
        </div>
          ...
          <div key={startup.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
            <div className="p-6">
              <div className="flex items-center mb-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    ...
                    <span className="px-2 py-1 bg-gray-100 text-gray-800 text-xs rounded-full">{startup.businessModel}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-2">
                <button className="flex-1 py-2 px-4 bg-primary text-white rounded-md hover:bg-primary/90 transition-colors">View Details</button>
                <button onClick={() => handleConnect(startup.id)} disabled={connectedIds.has(startup.id)} className={`flex-1 py-2 px-4 rounded-md transition-colors ${connectedIds.has(startup.id) ? 'bg-gray-200 text-gray-600 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700'}`}>
                  {connectedIds.has(startup.id) ? 'Requested' : 'Connect'}
                </button>
              </div>
              {startup.tags && startup.tags.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {startup.tags.map(tag => (<span key={tag} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">{tag}</span>))}
                </div>
              )}
            </div>
          </div>
        ))}
        {filtered.length === 0 && (<div className="col-span-full text-center text-gray-600">No startups match your filters.</div>)}
      </div>
    </div>
  );
};
